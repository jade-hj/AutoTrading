"""
Agent C — Groq / Gemma2-9b (시장 심리 & 수급 전문)
...
class GeminiAgent(BaseAgent):
    """시장 심리 & 수급 전문 Agent (Groq Gemma2-9b)."""

    def __init__(self):
        super().__init__("Gemma(시장심리)")
    ...
    def _system_prompt(self) -> str:
        return (
            "당신은 한국 주식 시장(KRX) 전문 시장 심리 분석가입니다.\n"
            "거래량 추이, 등락률, 수급 흐름을 중심으로 시장 심리와 모멘텀을 분석하여 "
...
## 당신의 역할
시장 심리 전문가로서 거래량 급증, 등락률 모멘텀, 수급 흐름을 분석하세요.
어떤 종목에 시장의 관심이 집중되고 있는지, 추세가 지속될지 판단하세요.
